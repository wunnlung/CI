...
//Added for headless -EGG -translate -CJG
int headless=0;
#ifdef _WINDOWS
...
message_t *TalkMsg[MAX_MSGS], *GameMsg[MAX_MSGS]; //Added game msg -CJG
...
//Scans message at given index from GameMsg array -CJG
...
//hudScore changed to display message at given index -CJG
...
//hudTimeLeft changed to display message at given index -CJG
...
//Removed wall detection flags -CJG
int Wall_here(int x_here, int y_here) {
    ...
    type = *mapptr;
    //if (flag_wf) {
      //  PaintBitmap(p_draw, BM_WALL_LEFT, WINSCALE(X(x - 1)), WINSCALE(Y(y + BLOCK_SZ)), WINSCALE(BLOCK_SZ), WINSCALE(BLOCK_SZ), 0);
      //  PaintBitmap(p_draw, BM_WALL_BOTTOM, WINSCALE(X(x)), WINSCALE(Y(y + BLOCK_SZ - 1)), WINSCALE(BLOCK_SZ), WINSCALE(BLOCK_SZ), 0);
       // PaintBitmap(p_draw, BM_WALL_RIGHT, WINSCALE(X(x + 1)), WINSCALE(Y(y + BLOCK_SZ)), WINSCALE(BLOCK_SZ), WINSCALE(BLOCK_SZ), 0);
      //  PaintBitmap(p_draw, BM_WALL_TOP, WINSCALE(X(x)), WINSCALE(Y(y + BLOCK_SZ + 1)), WINSCALE(BLOCK_SZ), WINSCALE(BLOCK_SZ), 0);
    //}
    if (type & BLUE_BIT)// {
       // if (flag_wd) {
         //   PaintBitmap(p_draw, BM_WALL_UR, WINSCALE(X(x)), WINSCALE(Y(y + BLOCK_SZ)), WINSCALE(BLOCK_SZ), WINSCALE(BLOCK_SZ), 0);
           // PaintBitmap(p_draw, BM_WALL_UL, WINSCALE(X(x)), WINSCALE(Y(y + BLOCK_SZ)), WINSCALE(BLOCK_SZ), WINSCALE(BLOCK_SZ), 0);
        //}
        return 1;
    //}
    return 0;
